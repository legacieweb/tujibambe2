...
const CarHire = () => {
  ...
  const prevImage = () => {
    if (!selectedVehicle?.gallery) return;
    setActiveImageIndex((prev) => (prev - 1 + selectedVehicle.gallery.length) % selectedVehicle.gallery.length);
  };
  ...
  const getVehicleImages = (vehicle) => {
    return vehicle.gallery && vehicle.gallery.length > 0 ? vehicle.gallery : [vehicle.image];
  };
          ...
          {filteredVehicles.length > 0 ? (
            <div className="vehicles-grid">
              {filteredVehicles.map((vehicle) => (
                <div key={vehicle.id || vehicle._id} className="vehicle-card">
            ...
            <div className="modal-flex">
              <div className="modal-image">
                <div className="vehicle-gallery-container">
                  <img src={getVehicleImages(selectedVehicle)[activeImageIndex]} alt={selectedVehicle.name} className="main-gallery-img" />
                  
                  {getVehicleImages(selectedVehicle).length > 1 && (
                    <div className="gallery-nav-btns">
                      <button className="gallery-nav-btn prev" onClick={prevImage}><ChevronLeft size={20} /></button>
                      <button className="gallery-nav-btn next" onClick={nextImage}><ChevronRight size={20} /></button>
                    </div>
                  )}

                  <div className="thumbnail-strip-mini">
                    {getVehicleImages(selectedVehicle).map((img, idx) => (
                      <div 
                        key={idx} 
                        className={`thumb-mini ${idx === activeImageIndex ? 'active' : ''}`}
